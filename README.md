# lie_group_integrators_double_spherical_pendulum
...
The point of departure is the formulation of the models as differential equations on manifolds. Assuming to be given either a Lie group acting transitively on the manifold :math:`\mathcal{M}` or a set of frame vector fields on :math:`\mathcal{M}`, we use them to describe the mechanical system and further to build the numerical integrator. We consider schemes of the types commonly known as Runge--Kutta--Munthe--Kaas methods and Commutator-free Lie group methods. 
